...
在添加文字样式之前我们需要将文字样式会用到的相关字体文件加入到前端CAD项目所引用的字体文件夹中，这一步是使前端CAD项目也能正常显示和编辑引用这些字体文件的字体样式。
...
执行上述加载字体文件的操作后，我们就可以直接调用 mxcad 实例对象中的 addTextStyle() 方法添加文字样式，再设置 drawTextStyle 属性将添加的文字样式设置为当前绘制文字样式。
