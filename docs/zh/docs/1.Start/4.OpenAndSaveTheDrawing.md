...
我们调用 mxdraw 提供的 createCanvasImageData() 方法，把当前图纸保存为jpg文件，其实质为将当前显示内容，变成image数据输出。具体详情请参考[API文档 createCanvasImageData()](https://help.mxdraw.com/mxdraw_docs/api/classes/MxDrawObject.html#createCanvasImageData)。
...
linux目录: `MxDrawCloudServer\Bin\Linux\MxCAD\mxcadassembly`
:::
...
/**
 * 保存文件的后端接口："http://localhost:1337/mxcad/savedwg"
 */
...
```
:::tip 注意
`http://localhost:1337/mxcad/savedwg`
