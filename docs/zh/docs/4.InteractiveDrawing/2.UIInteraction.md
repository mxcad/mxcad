...
在一些图纸或设计中，可能初始就存在一定的旋转角度，例如在CAD设计中，某些坐标系已经经过旋转或者偏移。为了使得用户交互的点能够自动适应这些变化，[MxCADUiPrPointTransform](../../api/classes/2d.MxCADUiPrPointTransform.md) 提供了一个解决方案：

* 图纸初始带旋转角度的情境：在图纸本身存在旋转的情况下，直接获取坐标点可能会产生偏差。通过使用该类，能够确保用户获取的点坐标是相对于图纸旋转后的正确位置，而不是在图纸原始坐标系中的错误位置。

* 自动坐标变换：当用户在图纸上点击或拖动时，MxCADUiPrPointTransform 会自动将用户交互的坐标变换成图纸上的坐标，并提供适配图纸旋转的正确位置，避免了手动调整坐标系的问题。
