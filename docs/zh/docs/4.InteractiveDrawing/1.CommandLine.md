...
搭建好基础的命令行UI界面后就需要监听用户输入，同时调用[MxFun.setCommandLineInputData()](https://help.mxdraw.com/mxdraw_docs/api/classes/MxFun.html#setCommandLineInputData)方法设置命令行消息数据；
调用[MxFun.listenForCommandLineInput()](https://help.mxdraw.com/mxdraw_docs/api/classes/MxFun.html#listenForCommandLineInput)方法来监听命令行消息动态更新的数据，在取点对象中设置的命令以及我们提供的图形对象中是有一套命令提示和参数化绘图。
...
注册命令需要依赖 mxdraw库 中提供的[MxFun.addCommand()](https://help.mxdraw.com/mxdraw_docs/api/classes/MxFun.html#addCommand)方法。下面以注册绘直线命令为例：
...
手动执行命令需要依赖 mxdraw库 中提供的[MxFun.sendStringToExecute()](https://help.mxdraw.com/mxdraw_docs/api/classes/MxFun.html#sendStringToExecute)方法。
