...
在项目初始化时通过 [MxCpp.App.addNetworkLoadingFont()](../../api/classes/2d.McAppType.md#addNetworkLoadingFont)、[MxCpp.App.addNetworkLoadingBigFont()](../../api/classes/2d.McAppType.md#addNetworkLoadingBigFont)、[MxCpp.App.addNetworkLoadingTrueTypeFont()](../../api/classes/2d.McAppType.md#addnetworkloadingtruetypefont) 方法来设置我们系统需要加载哪些字体，方便后面我们修改字体时加载使用。
...
我们可以将目标字体文件拷贝到云图开发包的`MxDrawCloudServer\Bin\MxCAD\Release\fonts` 目录下，在 mxcad 项目中添加上图纸需要的目标字体文件，这一步是使转换程序能够成功转换图纸。

 ![Alt text](../../../assets/img/%E6%B7%BB%E5%8A%A0%E5%AD%97%E4%BD%93%E6%96%87%E4%BB%B6-mxcad.jpg)

再将相关字体文件加入到前端项目所引用的字体文件夹中，这一步是使前端CAD项目也能正常显示和编辑引用这些字体文件的字体样式。
