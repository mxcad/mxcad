...
在上述步骤中我们已经获取到了导出图片在图纸上的范围，以及导出的大小，因此接下来我们可以直接调用 [McObject.mxdraw.createCanvasImageData()](https://help.mxdraw.com/mxdraw_docs/api/classes/MxDrawObject.html#createCanvasImageData) 方法来导出目标图片。其中，如果不设置导出图纸范围则将默认导出视区范围内的图纸内容。
