...
重绘块表记录中的对象，在修改了块表记录中的对象后，显示效果对时，可能需要调用一下这个函数。
...
| Name | Type | Description |
| :------ | :------ | :------ |
| `sSaveProgramUrl` | `string` | 后端 POST请求接口, 接口具体实现: 先下载MxDraw云图开发包<https://www.mxdraw.com/download.html>并解压 找到MxDrawCloudServer\Bin\MxCAD\MxCADSaveFile\server.js 可以找到对应的实现接口"/mxcad/savedwg", "/mxcad/savedwg":保存dwg文件到服务器 "/mxcad/savefile":保存mxweb文件到服务器 "/mxcad/savepdf":保存pdf文件到服务器 如果没有找到则在该项目下寻找这个接口的定义, 必须符合该接口的定义才能作为参数 |
...
| Name | Type | Description |
| :------ | :------ | :------ |
| `val` | `object` | 属性设置内容. 1. DisplayPrecision：number 显示精度设置，默认为0,可以取0 ~1000,1000为最高精度了 2. EnableUndo:boolean 启用undo功能，默认是没有启动的 3. ShowCoordinate:boolean 是否显示XY坐标轴 4. BlackWhiteDisplay:boolean 是否黑白显示 |
