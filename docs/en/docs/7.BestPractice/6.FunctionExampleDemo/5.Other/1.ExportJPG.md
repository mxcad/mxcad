...
In the above steps, we have obtained the scope and size of the exported picture on the drawing. So now we can call directly [McObject.mxdraw.createCanvasImageData()](https://mxcad.github.io/mxdraw/api/classes/MxDrawObject.html#createCanvasImageData) method to export the target image. If the drawing range is not set, the drawing content in the view range is exported by default.
