...
Passed during project initialization [MxCpp.App.addNetworkLoadingFont()](../../api/classes/2d.McAppType.md#addNetworkLoadingFont)、[MxCpp.App.addNetworkLoadingBigFont()](../../api/classes/2d.McAppType.md#addNetworkLoadingBigFont)、[MxCpp.App.addNetworkLoadingTrueTypeFont()](../../api/classes/2d.McAppType.md#addnetworkloadingtruetypefont) method to set our system needs to load which fonts, convenient for later when we modify the font loading use.
...
We can copy the target font file to the `MxDrawCloudServer\Bin\MxCAD\Release\fonts` directory in the mxcad project to add the target font file required by the drawing , This step is to ensure that the conversion program can successfully convert the drawings.

 ![Alt text](../../../assets/en/img/%E6%B7%BB%E5%8A%A0%E5%AD%97%E4%BD%93%E6%96%87%E4%BB%B6-mxcad.jpg)

Then, add the relevant font files to the font folder referenced by the front-end project. This step is necessary to ensure that the front-end CAD project can also display and edit the font styles that reference these font files properly.
...
The following takes setting the default initial font file for loading a CAD project as an example:
```ts
import { MxFun } from "mxdraw";
import { MxCpp } from "mxcad";
...
### 1. Close the mobile terminal three-view
