...
```
## The total area of the drawing viewing zone

We can obtain the current actual total viewing area range of the drawing by getting the bounding box position of the current space block in the drawing database.

```ts
import { MxCpp } from "mxcad";

const {minPt, maxPt} = MxCpp.getCurrentMxCAD().getDatabase().currentSpace.getBoundingBox();
console.log('The minimum point of the actual viewing area:' + minPt);
console.log('The point with the largest actual viewing area:' + maxPt);
```

## The view area is displayed in black and white

We can set the BlackWhiteDisplay property of the mxcad object to display the CAD drawing in white, that is, to set the view area to be displayed in black and white. If this property is set after opening the drawing, the drawing needs to be opened again to make the program re-render the target drawing.
...
We can call the createLayout() method of the mxcad instance object to enter the name of the new layout to create a new layout.
...
We can call the deleteLayout() method of the mxcad instance object to delete the layout with the specified name.
...
We can call the layoutRename() method of the mxcad instance object to input the original layout name that needs to be renamed and the new layout name.
