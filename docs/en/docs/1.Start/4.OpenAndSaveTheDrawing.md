...
```
:::tip Note
` http://localhost:1337/mxcad/savefiledwg ` interface provided by the corresponding Node in practical development kit is cloud services
:::
...
mxcad.saveFile(void 0, (data)=> {
      ...
      fetch('http://localhost:1337/mxcad/savefiledwg', {
          method:'POST',
          body: file
      })
}, false, true)
