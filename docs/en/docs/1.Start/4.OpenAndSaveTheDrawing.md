...
We call the createCanvasImageData() method provided by mxdraw to save the current drawing as a jpg file, whose essence is to turn the current display content into image data output. For details, please refer to [API Documentation createCanvasImageData()](https://mxcad.github.io/mxdraw/api/classes/MxDrawObject.html#createCanvasImageData)。
...
```
:::tip Note
`http://localhost:1337/mxcad/print_to_pdf`
...
linux directory: 'MxDrawCloudServer\Bin\Linux\MxCAD\mxcadassembly'
:::
...
```
:::tip Note
` http://localhost:1337/mxcad/savedwg ` interface provided by the corresponding Node in practical development kit is cloud services
:::
...
mxcad.saveFile(void 0, (data)=> {
      ...
      fetch('http://localhost:1337/mxcad/savedwg', {
          method:'POST',
          body: file
      })
}, false, true)
