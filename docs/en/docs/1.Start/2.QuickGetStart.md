...
Among them, the setting for obtaining the path of loading the wasm-related files requires choosing between the multi-threaded mode or the single-threaded mode based on the actual situation. For detailed information, please refer to [Mode Description](#description-of-multi-thread-and-single-thread-mode).
