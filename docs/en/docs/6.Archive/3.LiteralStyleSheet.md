...
Before adding the text styles, we need to add the font files that will be used for the text styles to the font folder referenced by the front-end CAD project. This step ensures that the front-end CAD project can also display and edit the text styles that reference these font files properly.

After adding the font files, we can notify the mxcad object to load these fonts during the initialization stage of the CAD project, or we can load the files in advance when they are needed. Different types of font files require different font loading APIs.
...
* Load TrueType font
```ts
// MxCpp.App.addNetworkLoadingTrueTypeFont(["syadobe"], ["Thought Original Bold Font"], ["syadobe.otf"]);
MxCpp.App.addNetworkLoadingTrueTypeFont(["Font Name"], ["Chinese Name of Font"], ["Font File"]);
```
Here is an example of setting the default initial font file for loading CAD projects:
```ts
import { MxFun } from "mxdraw";
import { MxCpp } from "mxcad";
// MxCAD creation successful，mxcad.on("init_mxcad")
MxFun.on("mxcadApplicationCreatedMxCADObject", (param) => {
    ...
    //MxCpp.App.addNetworkLoadingTrueTypeFont(["simsun", "syadobe"], ["SimSun Font", "Thought Original Bold Font"], ["stadobe.otf", "syadobe.otf"]);
    MxCpp.App.addNetworkLoadingTrueTypeFont(["syadobe"], ["Thought Original Bold Font"], ["syadobe.otf"]);
})
```

After performing the operation of loading the font file mentioned above, we can directly call the addTextStyle() method in the mxcad instance object to add the text style, and then set the drawTextStyle property to set the added text style as the current drawing text style.
