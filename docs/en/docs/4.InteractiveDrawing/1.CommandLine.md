...
Once you've built the basic command line UI, you need to listen to user input, At the same time call [MxFun setCommandLineInputData()](https://mxcad.github.io/mxdraw/api/classes/MxFun.html#setCommandLineInputData ) method to set the command line message data;
Call [MxFun listenForCommandLineInput()](https://mxcad.github.io/mxdraw/api/classes/MxFun.html#listenForCommandLineInput) method to listen for dynamically updated data from command line messages, the commands set in the fetch point object and the graphic objects we provide are a set of command prompts and parametric plots.
...
```
Registered command need to rely on mxdraw libraries provide [MxFun. AddCommand()](https://mxcad.github.io/mxdraw/api/classes/MxFun.html#addCommand). The following takes the registration line drawing command as an example:
...
Manual command execution relies on the mxdraw library Provided in the [MxFun sendStringToExecute()](https://mxcad.github.io/mxdraw/api/classes/MxFun.html#sendStringToExecute).
...
4. Remove registration command
```ts
import { MxFun } from "mxdraw"

MxFun.removeCommand("Registered command name");
```
