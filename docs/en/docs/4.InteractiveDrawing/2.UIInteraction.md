...
### Transform coordinates into points

In some drawings or designs, there may be a certain rotation Angle initially. For instance, in CAD designs, some coordinate systems have already been rotated or offset. In order to enable the points of user interaction to automatically adapt to these changes, [MxCADUiPrPointTransform](../../api/classes/2d.MxCADUiPrPointTransform.md) offers a solution：

* The situation where the drawing initially has a rotation Angle: In the case where the drawing itself has rotation, directly obtaining the coordinate points may cause deviations. By using this class, it can be ensured that the point coordinates obtained by the user are the correct positions after rotation relative to the drawing, rather than the incorrect positions in the original coordinate system of the drawing.

* Automatic coordinate transformation: When the user clicks or drags on the drawing, MxCADUiPrPointTransform will automatically transform the user's interactive coordinates into those on the drawing and provide the correct position to adapt to the drawing's rotation, avoiding the problem of manually adjusting the coordinate system。
