  ...
  if (fileExt === ".dwg" || fileExt === ".dxf") {
    ...
    try {
      ...
      if (!res.ok) throw new Error(`请求失败: ${res.status}`);
            const baseUrl = import.meta.env.VITE_API_BASE; // 从环境变量取基础地址
      const mxwebFullUrl = `${baseUrl}/demo/${fileNameWithoutExt}.mxweb`;
      mxcad.openWebFile("http://localhost:3000/demo/" + fileName + ".mxweb");
    } catch (err) {
      console.error(err);
      alert("上传或转换失败");
    }
